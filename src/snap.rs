...
use crate::{
    dbgp,
    event::LimitOrder,
    orderbook::{HalfBook, Order, OrderBook, Side},
};
...
fn filter_stale_synth_orders(level: &VecDeque<Order>, cur_epoch: u64) -> VecDeque<Order> {
    let filtered_result: VecDeque<Order> = level
        .iter()
        .filter(|order| {
            !order.is_synth || cur_epoch.abs_diff(order.send_time) < 10_000_000_000 && order.qty > 0
        })
...
fn merge_level_with_qty(level: &mut VecDeque<Order>, qty: u32, cur_epoch: u64) -> VecDeque<Order> {
    ...
    let filtered_result: VecDeque<Order> = level
        ...
        .filter(|order| {
            (order.is_synth
                && cur_epoch.abs_diff(order.send_time) < 10_000_000_000
                && order.qty > 0)
                || (!order.is_synth && order.fill_time != u64::MAX)
        })
        .copied() // Clone the orders to create a new VecDeque
        .collect();
...
fn merge_halfbook_with_snap(
    halfbook: &mut HalfBook,
    snap: &Snap,
    side: Side,
    boundary_price: u32,
) -> HalfBook {
    ...
    while i_book < book_len && i_snap < snap_len {
        ...
        let cur_book_price = halfbook.price_levels[i_book]
            .front()
            .map_or(u32::MAX, |order| order.price);
        let cur_snap_price = snap.vec[i_snap].price;

        let snap_ahead = ((side == Side::Ask) && (cur_book_price < cur_snap_price))
            || ((side == Side::Bid) && (cur_book_price > cur_snap_price));

        if snap_ahead {
            // Copy the whole level into the new order book
            let filtered_level =
                filter_stale_synth_orders(&halfbook.price_levels[i_book], snap.exch_epoch);
            ...
            let merged_level = merge_level_with_qty(
                &mut halfbook.price_levels[i_book],
                snap.vec[i_snap].qty,
                snap.exch_epoch,
            );
...
fn next_snap_fp(snap: &Snap, ob: &mut OrderBook) -> OrderBook {
    ...
    let ask_book =
        merge_halfbook_with_snap(&mut ob.ask_book, &ask_snap, Side::Ask, bid_orders[0].price);
    let bid_book =
        merge_halfbook_with_snap(&mut ob.bid_book, &bid_snap, Side::Bid, ask_orders[0].price);

    let best_ask_price = ask_book.price_levels[0]
        .front()
        .map_or(u32::MAX, |order| order.price);
    //let best_bid_price = bid_book.price_levels[0].front().map_or(u64::MAX, |order| order.price);
    let best_bid_price = bid_book.price_levels[0]
        .front()
        .map_or(u32::MAX, |order| order.price);
