        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for EventVisitor {
                ...
                A: MapAccess<'de>,
            {
                ...
                let side = Side::Bid;
                let price = bid_1_price
                    .ok_or_else(|| Error::missing_field("bid_1_price"))?;
                let qty = bid_1_qty
                    .ok_or_else(|| Error::missing_field("bid_1_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = bid_2_price
                    .ok_or_else(|| Error::missing_field("bid_2_price"))?;
                let qty = bid_2_qty
                    .ok_or_else(|| Error::missing_field("bid_2_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = bid_3_price
                    .ok_or_else(|| Error::missing_field("bid_3_price"))?;
                let qty = bid_3_qty
                    .ok_or_else(|| Error::missing_field("bid_3_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = bid_4_price
                    .ok_or_else(|| Error::missing_field("bid_4_price"))?;
                let qty = bid_4_qty
                    .ok_or_else(|| Error::missing_field("bid_4_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = bid_5_price
                    .ok_or_else(|| Error::missing_field("bid_5_price"))?;
                let qty = bid_5_qty
                    .ok_or_else(|| Error::missing_field("bid_5_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = bid_6_price
                    .ok_or_else(|| Error::missing_field("bid_6_price"))?;
                let qty = bid_6_qty
                    .ok_or_else(|| Error::missing_field("bid_6_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = bid_7_price
                    .ok_or_else(|| Error::missing_field("bid_7_price"))?;
                let qty = bid_7_qty
                    .ok_or_else(|| Error::missing_field("bid_7_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = bid_8_price
                    .ok_or_else(|| Error::missing_field("bid_8_price"))?;
                let qty = bid_8_qty
                    .ok_or_else(|| Error::missing_field("bid_8_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = bid_9_price
                    .ok_or_else(|| Error::missing_field("bid_9_price"))?;
                let qty = bid_9_qty
                    .ok_or_else(|| Error::missing_field("bid_9_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = bid_10_price
                    .ok_or_else(|| Error::missing_field("bid_10_price"))?;
                let qty = bid_10_qty
                    .ok_or_else(|| Error::missing_field("bid_10_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let side = Side::Ask;
                let price = ask_1_price
                    .ok_or_else(|| Error::missing_field("ask_1_price"))?;
                let qty = ask_1_qty
                    .ok_or_else(|| Error::missing_field("ask_1_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = ask_2_price
                    .ok_or_else(|| Error::missing_field("ask_2_price"))?;
                let qty = ask_2_qty
                    .ok_or_else(|| Error::missing_field("ask_2_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = ask_3_price
                    .ok_or_else(|| Error::missing_field("ask_3_price"))?;
                let qty = ask_3_qty
                    .ok_or_else(|| Error::missing_field("ask_3_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = ask_4_price
                    .ok_or_else(|| Error::missing_field("ask_4_price"))?;
                let qty = ask_4_qty
                    .ok_or_else(|| Error::missing_field("ask_4_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = ask_5_price
                    .ok_or_else(|| Error::missing_field("ask_5_price"))?;
                let qty = ask_5_qty
                    .ok_or_else(|| Error::missing_field("ask_5_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = ask_6_price
                    .ok_or_else(|| Error::missing_field("ask_6_price"))?;
                let qty = ask_6_qty
                    .ok_or_else(|| Error::missing_field("ask_6_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = ask_7_price
                    .ok_or_else(|| Error::missing_field("ask_7_price"))?;
                let qty = ask_7_qty
                    .ok_or_else(|| Error::missing_field("ask_7_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = ask_8_price
                    .ok_or_else(|| Error::missing_field("ask_8_price"))?;
                let qty = ask_8_qty
                    .ok_or_else(|| Error::missing_field("ask_8_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = ask_9_price
                    .ok_or_else(|| Error::missing_field("ask_9_price"))?;
                let qty = ask_9_qty
                    .ok_or_else(|| Error::missing_field("ask_9_qty"))?;
                vec.push(LimitOrder { side, price, qty });
                let price = ask_10_price
                    .ok_or_else(|| Error::missing_field("ask_10_price"))?;
                let qty = ask_10_qty
                    .ok_or_else(|| Error::missing_field("ask_10_qty"))?;
                vec.push(LimitOrder { side, price, qty });
